...
class Config(configparser.RawConfigParser):
    ...
    def section(self, section, key_type=str, value_type=str):
        try:
            ...
            return dict(
                map(
                    lambda kv: (key_type(kv[0]), value_type(kv[1])),
    ...
    def sections(self, prefix=None):
        for section in configparser.RawConfigParser.sections(self):
        ...
        for option in self.__options__:
            if hasattr(namespace, option):
                ...
                if isinstance(value, str):
                    for action in filter(lambda a: a.dest == option, parser._actions):
...
def load_options():
    ...
    options.bindings = {}
    options.bindings["global"] = config.section("bindings", key_type=parse_button_combo)
    for name, section in config.sections("bindings"):
        options.bindings[name] = config.section(section, key_type=parse_button_combo)
