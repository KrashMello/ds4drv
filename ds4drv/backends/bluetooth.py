...
class BluetoothDS4Device(DS4Device):
    ...
    def __init__(self, addr, ctl_sock, int_sock):
        ...
        super(BluetoothDS4Device, self).__init__(addr.upper(), addr, "bluetooth")
...
class BluetoothBackend(Backend):
    ...
    def setup(self):
        ...
        try:
            ...
            raise BackendError(
                "'hcitool clock' returned error. Make sure "
                "your bluetooth device is powered up with "
                "'hciconfig hciX up'."
            )
        except OSError:
            raise BackendError(
                "'hcitool' could not be found, make sure you "
    ...
    def scan(self):
        ...
        try:
            res = subprocess.check_output(
                ["hcitool", "scan", "--flush"], stderr=subprocess.STDOUT
            )
        except subprocess.CalledProcessError:
            raise BackendError(
                "'hcitool scan' returned error. Make sure "
                "your bluetooth device is powered up with "
                "'hciconfig hciX up'."
            )
    ...
    def devices(self):
        ...
        while True:
            ...
            try:
                ...
                self.logger.error("Error while scanning for devices: {0}", err)
                return
            except DeviceError as err:
                self.logger.error("Unable to connect to detected device: {0}", err)
